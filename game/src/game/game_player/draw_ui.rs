...
fn load_accuracy_textures(texture_creator: &TextureCreator<WindowContext>) -> Result<AccuracyTextures, String> {
    Ok(AccuracyTextures {
...
pub fn draw_ui(canvas: &mut Canvas<Window>, notes: Vec<DisplayedSongNote>, other: UIContent) {
    ...
    if let Some(accuracy) = other.accuracy {
        ...
        let width = 120;
        let height = (Rational32::new(accuracy_texture.query().height as i32 * width as i32, accuracy_texture.query().width as i32)).to_integer();
        let x = guideline_x + (note_width as i32 / 2) - (width / 2);
        let y = guideline_y + (note_height as i32 / 2) - (height / 2);

        canvas.copy(&accuracy_texture, None, Rect::new(x, y, width as u32, height as u32));
    }
}